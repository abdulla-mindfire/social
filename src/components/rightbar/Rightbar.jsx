...
export default function Rightbar({ user }) {
  ...
  const fetchFriendsToFollow = async () => {
    let headers = {
      "Authorization": `Bearer ${currentUser?.access}`
    }
    const res = await axios.get(API_BE + `account/friends-to-follow`, { headers: headers })
    if (res.status === 200) {
      setFriendsToFollow(res.data)
    }
  }
  ...
  const ProfileRightbar = () => {
    return (
      <>
        ...
        <h4 className="rightbarTitle">User information {user?.user?.id == currentUser?.id && <Link style={{ color: "black" }} to={`/profile/${user?.user?.id}/edit`} ><Edit /></Link>}</h4>
